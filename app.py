...
def display_name_for(user):
    ...
    `display_name` is the free-form name people choose. When it is unset,
    fall back to the local part of their email rather than the `username`
    column: that is generated by a database trigger and never shown, so
    greeting somebody by a handle they never picked would be worse than
    guessing from the address they typed.
    """
    if not user:
        return ""
    ...
    return email.split("@")[0] if email else (user["username"] or "")
...
def register():
    ...
    if request.method == "POST":
        ...
        # No username is asked for: the on_auth_user_created trigger derives a
        # unique one from the email, and nothing user-facing displays it.
        if not EMAIL_RE.match(email):
            error = "Please enter a valid email address."
        elif len(password) < 8:
            error = "Password must be at least 8 characters."
        else:
            auth, err = supabase_auth.sign_up(email, password)
            if err:
                error = err
...
def profile():
    ...
    if request.method == "POST":
        ...
        # The form no longer submits a username. Validating one here would
        # reject every save, since the field does not exist any more.
        if len(display) > 40:
            ...
            db.update_profile(user["id"], avatar_url=avatar_url,
...
def profile_delete():
    ...
    # Confirm on the email, which is the identity people actually know. The
    # username is generated by a database trigger and never shown, so asking
    # for it would be a puzzle rather than a safeguard.
    if typed.lower() != (user["email"] or "").lower():
